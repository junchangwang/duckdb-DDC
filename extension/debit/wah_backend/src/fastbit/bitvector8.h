...
class ibis::bitvector8 : public bitvector_base {
public:
    typedef uint8_t word_t;      /// the basic unit of data storage is 8-bit.

    // default construct 
    bitvector8() : nbits(0), nset(0), active(), m_vec() {};

    bitvector8(const bitvector8& bv);
    ...
    void append_fill(int val, word_t cnt);          /// = bitvector.h append_counter()
    
    void appendWord(word_t w);                      /// = bitvector.h append_Word()

    void append_bits(int val, uint64_t n);         /// = bitvector.h append_Fill()
    ...
    void appendActive() {
    	if (active.nbits > 0) append_active();
    }
    
    bitvector_base& copy(const bitvector_base& bv){
        const bitvector8 &tmp = dynamic_cast<const bitvector8 &>(bv);
        clear();
        nbits = tmp.nbits;
        nset = tmp.nset;
        active = tmp.active;
    ...
    std::vector<uint8_t> decode(std::vector<uint8_t>& append_rowids, Table_config* config);
    ...
    struct run8 {
        ...
        /// construct
        run8() : isFill(0), fillBit(0), nWords(0), it(0), wordPos(0), vecPos(-1), nbits(0), bitPos(0) { };
        ...
        /// Reduce the run size by len words. 
        ...
        /// Reduce the run size by len bits. 
    ...
    inline void copy_runs(run8& it, word_t& nw); // copy nw words
    ...
    void showindex(){            
        ...
        //}
        //std::cout << endl; 
        std::cout << "active info: " << active.nbits << " " << std::bitset<8> (active.val) << std::endl;
        do_cnt();
        std::cout << "read btv8: "<< nbits << " " << nset << std::endl << std::endl;      
    }
...
private:
    ...
    void or_c2(const bitvector8& rhs, bitvector8& res) const;

    void or_c1(const bitvector8& rhs, bitvector8& res) const;

    void or_c0(const bitvector8& rhs);

    void or_d1(const bitvector8& rhs);

    void or_d2(const bitvector8& rhs, bitvector8& res) const;

    void and_c2(const bitvector8& rhs, bitvector8& res) const;

    void and_c1(const bitvector8& rhs, bitvector8& res) const;

    void and_c0(const bitvector8& rhs);

    void and_d1(const bitvector8& rhs);

    void and_d2(const bitvector8& rhs, bitvector8& res) const;

    void xor_c2(const bitvector8& rhs, bitvector8& res) const;

    void xor_c1(const bitvector8& rhs, bitvector8& res) const;

    void xor_c0(const bitvector8& rhs);

    void xor_d1(const bitvector8& rhs);

    void xor_d2(const bitvector8& rhs, bitvector8& res) const;

    void copy_comp(std::vector<word_t>& tmp) const;

    inline void copy_fill(std::vector<word_t>::iterator& jt, run8& it);
    
    inline void copy_runsn(run8& it, word_t& nw); // copy nw words and negate

    inline void copy_runs(std::vector<word_t>::iterator& jt, run8& it, word_t& nw);

    inline void copy_runsn(std::vector<word_t>::iterator& jt, run8& it, word_t& nw);

    inline void copy_runs(run8& it, word_t& nw, const std::vector<std::pair<int, uint64_t>>& index, int& bi_idx_pos); // copy nw words
...
/// words to copy.
inline void ibis::bitvector8::copy_runs(run8& it, word_t& nw) {
    ...
    nbits -= MAXBITS * nw;
} // ibis::bitvector8::copy_runs
#endif

/// Copy the complements of a set of consecutive runs.  It assumes
/// active to be empty.
inline void ibis::bitvector8::copy_runsn(run8& it, word_t& nw) {
    ...
    nbits -= MAXBITS * nw;
} // ibis::bitvector8::copy_runsn

/// Copy the fill in "run it" as literal words.
/// This implementation relies on the fact that the iterator jt is actually
/// a bare pointer.
inline void ibis::bitvector8::copy_fill
(std::vector<ibis::bitvector8::word_t>::iterator& jt, run8& it) {
    if (it.fillBit == 0) {
        while (it.nWords > 3) {
            *jt = 0;
            jt[1] = 0;
...
/// counted first.
inline void ibis::bitvector8::copy_runs
(std::vector<ibis::bitvector8::word_t>::iterator& jt, run8& it, word_t& nw) {
...
inline void ibis::bitvector8::copy_runsn
(std::vector<ibis::bitvector8::word_t>::iterator& jt, run8& it, word_t& nw) {
    while (nw >= it.nWords) {
...
inline void ibis::bitvector8::copy_runs(run8& it, word_t& nw, const std::vector<std::pair<int, uint64_t>>& index,
    int& bi_idx_pos) {
    // deal with the first word -- attach it to the last word in m_vec
    if (it.isFill != 0) {
        if (it.nWords > 1) {
