...
extern unsigned int INDEX_WORDS;
...
@ingroup FastBitIBIS
*/
class FASTBIT_CXX_DLLSPEC ibis::bitvector : public bitvector_base {
public:
    ...
    inline bitvector_base &copy(const bitvector_base &bv);
    ...
    void setBit(const uint64_t i, int val, Table_config *config);

    int getBit(const uint64_t i, Table_config *config) const;

    int getBitWithIndex(const word_t i) const;
    ...
    void operator&=(const bitvector_base& rhs) {
        *this &= dynamic_cast<const bitvector&>(rhs);
    }
    ...
    void read_bmz(const char *fn, std::pair<uint32_t, uint32_t> place);
    ...
    void compress(float expect_compress_ratio = 0.382);
    ...
    bool isCompressed() const {
        return (m_vec.size() * MAXBITS < nbits);
    }
    ...
    // functions for nicolas
    //std::vector<std::pair<int, int>> index;

    void buildIndex();

    void merge(const bitvector &rhs);
    ...
    struct run {
        ...
        run() : isFill(0), fillBit(0), nWords(0), it(0), wordPos(0), vecPos(-1) { };
...
private:
    ...
    inline void copy_runs(run &it, word_t &nw, const std::vector<std::pair<int, uint64_t>> &index,int &bi_idx_pos); // copy nw words
...
public:
    ...
    pit(const ibis::bitvector &bv) : curr(0xFFFFFFFFU) {
        init(bv);
    }
...
inline bitvector_base &ibis::bitvector::copy(const bitvector_base &bv){
    const bitvector &tmp = dynamic_cast<const bitvector &>(bv);
    clear();
    nbits = tmp.nbits;
    nset = tmp.nset;
    active = tmp.active;
    m_vec.deepCopy(tmp.m_vec);
    return *this;
}
...
inline void ibis::bitvector::copy_runs(run &it, word_t &nw, const std::vector<std::pair<int, uint64_t>> &index,
                                       int &bi_idx_pos) {
    // deal with the first word -- attach it to the last word in m_vec
    if (it.isFill != 0) {
        if (it.nWords > 1) {
