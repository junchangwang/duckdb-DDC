...
unsigned int INDEX_WORDS_16 = 6666;

// static members
const uint64_t ibis::bitvector16::MAXBITS = 15;
const uint64_t ibis::bitvector16::SECONDBIT = 14;                     
const ibis::bitvector16::word_t ibis::bitvector16::ALLONES =    /// 0111111111111111
(((word_t)1 << ibis::bitvector16::MAXBITS) - 1); 
const ibis::bitvector16::word_t ibis::bitvector16::MAXCNT =     /// 0011111111111111  16383
(((word_t)1 << ibis::bitvector16::SECONDBIT) - 1);
const ibis::bitvector16::word_t ibis::bitvector16::FILLBIT =    /// 0100000000000000
((word_t)1 << bitvector16::SECONDBIT);
const ibis::bitvector16::word_t ibis::bitvector16::HEADER0 =    /// 1000000000000000
((word_t)2 << ibis::bitvector16::SECONDBIT);
const ibis::bitvector16::word_t ibis::bitvector16::HEADER1 =    /// 1100000000000000
((word_t)3 << ibis::bitvector16::SECONDBIT);
const ibis::bitvector16::word_t ibis::bitvector16::FULLBIT0 = 49151;   /// 101111111111111
const ibis::bitvector16::word_t ibis::bitvector16::FULLBIT1 = 65535;   /// 111111111111111



/// Copy constructor. 
ibis::bitvector16::bitvector16(const bitvector16& bv)
    : nbits(bv.nbits), nset(bv.nset), active(bv.active), m_vec(bv.m_vec) {
}

/// Replace a single bit at position ind with val.
void ibis::bitvector16::setBit(const uint64_t ind, int val, Table_config* config) {
    //std::cout << "16-bit setbit" << std::endl;
    ...
    if (m_vec.size() * MAXBITS == nbits) { 
        ...
        if (ind0 > 0) { // has not found the right location yet
            ...
            else {     // extends the current bit std::vector
...
/// (okay)
int ibis::bitvector16::getBit(const uint64_t ind, Table_config* config) const {
    if (ind >= size()) {
        return 0;
    }
...
/// use in getBit function 
int ibis::bitvector16::getBitWithIndex(const uint64_t ind) const {
    uint64_t jnd = ind;
    std::vector<word_t>::const_iterator it = m_vec.begin();
...
void ibis::bitvector16::appendWord(word_t w) {
    word_t nb1, nb2;
...
/// push last full active into m_vec and append a new active
void ibis::bitvector16::append_active() {
    if (m_vec.empty()) {
...
void ibis::bitvector16::append_fill(int val, word_t cnt) {
    if (cnt > MAXCNT)
        return;
...
// append n val-bits 
void ibis::bitvector16::append_bits(int val, uint64_t n) {
    if (n == 0) return;
    if (active.nbits > 0) {
...
void ibis::bitvector16::adjustSize(uint64_t nv, uint64_t nt) {
...
} // ibis::bitvector16::adjustSize

/// count nbits and nset
uint64_t ibis::bitvector16::do_cnt() const throw() {
    uint64_t ns = 0;
    uint64_t nb = 0;
    if (m_vec.empty() == 0){
...
uint64_t ibis::bitvector16::cnt_ones(word_t val) const{    
...
/// append a bit(OKAY)
ibis::bitvector16 &ibis::bitvector16::operator+=(int b) {
    active.append(b);
    if (active.is_full())
        append_active();
    return *this;
...
//
void ibis::bitvector16::write(const char* fn) const {
    if (fn == 0 || *fn == 0) return;
    ...
    if (out == 0) {
        LOGGER(ibis::gVerbose > 0)
            ...
            << fn << "\" to write the bit std::vector ... "
    ...
    ierr = fwrite((const void*)&(*m_vec.begin()), sizeof(word_t), m_vec.size(), out);
...
void ibis::bitvector16::read(const char* fn) {
    if (fn == 0 || *fn == 0) 
        return;
    ...
    for(int i = 0; i < img.size() / 2 - 2; i++){
        //std::cout << bitset<16>(img[i]) << std::endl;
        m_vec.push_back(img[i]);
        //m_vec[i] = img[i];
        //std::cout << bitset<16>(m_vec[i]) << std::endl;
    }   
    //std::cout << bitset<16>(img[img.size() - 2]) << std::endl;
    //std::cout << bitset<16>(img.back()) << std::endl << std::endl; 
    //std::cout << "img.size:" << img.size() << std::endl;
    //std::cout << "m_vec.size:" << m_vec.size() << std::endl;
    
    if(m_vec.size() > 1){
        //std::cout << this->m_vec.size() << std::endl << std::endl;
        if(img[img.size() / 2 - 1] > 0){
            active.val = img[img.size() / 2 - 2];
            active.nbits = img[img.size() / 2 -1];
        }
        else{
            active.val = 0;
            active.nbits = 0;
    ...
    img.clear();
    /*
    for(std::vector<word_t>::const_iterator it = m_vec.begin(); it != m_vec.end(); it++){
        std::cout << bitset<16>(*it) << std::endl;
    }
    std::cout << bitset<16>(active.val) << std::endl ;*/
    //std::cout << bitset<16>(active.nbits) << std::endl ;  
}

std::vector<uint16_t> ibis::bitvector16::decode(std::vector<uint16_t> &append_rowids, Table_config *config) {
    std::vector<uint16_t> rids(m_vec.size());
...
/*void ibis::bitvector16::countWords(){
    int num_fill = 0;
    int num_lite = 0;
    int tmp = 0;
    if (m_vec.empty() == 0) 
...
void ibis::bitvector16::compress() {
    ...
    if (m_vec.size() != tmp.size())
        m_vec.swap(tmp);  // take on the new std::vector
}

void ibis::bitvector16::decompress() {
    if (nbits == 0 && m_vec.size() > 0)
        nbits = do_cnt();
    // std::cout << "btv16 nbits: " << nbits <<endl;
    if (m_vec.size() * MAXBITS == nbits) {// already uncompressed
        std::cout << "the btv16 has been decompressed !" << std::endl;
        return;
    }
    std::vector<word_t> tmp;
...
/// Decompress the current content to an std::vector<word_t>.
void ibis::bitvector16::decompress(std::vector<ibis::bitvector16::word_t>& tmp)
const {
...
} // ibis::bitvector16::decompress

/// Decompress the current content to an std::vector<word_t> and complement
/// every bit.
void ibis::bitvector16::copy_comp(std::vector<ibis::bitvector16::word_t>& tmp) const {
...
void ibis::bitvector16::operator&=(const ibis::bitvector16& rhs) {
    ...
    LOGGER(ibis::gVerbose > 30 || ((1U << ibis::gVerbose) >= bytes() &&
        ...
        << "operator&=: (A&B)=" << *this;
#endif
} // ibis::bitvector16::operator&=

/// The is the in-place version of the bitwise OR (|) operator.  This
/// bitvector16 is modified to store the result of the operation.
void ibis::bitvector16::operator|=(const ibis::bitvector16& rhs) {
...
void ibis::bitvector16::or_c0(const ibis::bitvector16& rhs) {
...
#endif
} // ibis::bitvector16::or_c0

// or operation where rhs is not compressed (not one word is a counter)
void ibis::bitvector16::or_c1(const ibis::bitvector16& rhs,
    ibis::bitvector16& res) const {
    res.clear();
    if (m_vec.size() == 1) {    /// only one WAH word
        std::vector<word_t>::const_iterator it = m_vec.begin();
        if (*it > HEADER1) {          // 1-fill
            res.m_vec = m_vec;
            res.nbits = nbits;
        ...
        while (i0 != m_vec.end()) {    // go through all words in m_vec
            if (*i0 > ALLONES) {       // i0 is compressed
                ...
                else { // i0 is 0-fill, so the result is *i1
                    i2 = i1 + s0;
                    for (; i1 < i2; i1++)
    ...
    lg() << "result or OR " << res;
#endif
} // ibis::bitvector16::or_c1

// or operation on two compressed bitvectors
void ibis::bitvector16::or_c2(const ibis::bitvector16& rhs,
    ibis::bitvector16& res) const {
    res.clear();
    if (m_vec.size() == 1) {
        std::vector<word_t>::const_iterator it = m_vec.begin();
...
#endif
} // ibis::bitvector16::or_d1

// or operation on two compressed bitvectors, generates uncompressed result
void ibis::bitvector16::or_d2(const ibis::bitvector16& rhs,
    ibis::bitvector16& res) const {
    ...
    LOGGER(ibis::gVerbose >= 0)
        ...
        << "\nResult of or_d2 " << res;
#endif
} // ibis::bitvector16::or_d2

// both operands of the 'and' operation are not compressed
void ibis::bitvector16::and_c0(const ibis::bitvector16& rhs) {
    nset = 0;
    // m_vec.nosharing(); // make sure *this is not shared!
    std::vector<word_t>::iterator i0 = m_vec.begin();
    std::vector<word_t>::const_iterator i1 = rhs.m_vec.begin();
...
#endif
} // ibis::bitvector16::and_c0

// and operation where rhs is not compressed (not one word is a counter)
void ibis::bitvector16::and_c1(const ibis::bitvector16& rhs,
    ibis::bitvector16& res) const {
    res.clear();
    if (m_vec.size() == 1) {
        std::vector<word_t>::const_iterator it = m_vec.begin();
...
#endif
} // ibis::bitvector16::and_c1

// bitwise and (&) operation -- both operands may contain compressed words
void ibis::bitvector16::and_c2(const ibis::bitvector16& rhs,
    ibis::bitvector16& res) const {
    int bi_idx_pos = 0;
    res.clear();
    if (m_vec.size() == 1) {
...
void ibis::bitvector16::and_d1(const ibis::bitvector16& rhs) {
    // m_vec.nosharing(); // make sure *this is not shared!
#if DEBUG + 0 > 1 || _DEBUG + 0 > 1
    {
        ibis::util::logger lg(4);
...
void ibis::bitvector16::and_d2(const ibis::bitvector16& rhs,
    ibis::bitvector16& res) const {
#if DEBUG + 0 > 1 || _DEBUG + 0 > 1
    ...
    lg() << "result of AND " << res;
#endif
} // ibis::bitvector16::and_d2

// both operands of the 'xor' operation are not compressed
void ibis::bitvector16::xor_c0(const ibis::bitvector16& rhs) {
    // m_vec.nosharing(); // make sure *this is not shared!
    nset = 0;
    std::vector<word_t>::iterator i0 = m_vec.begin();
    std::vector<word_t>::const_iterator i1 = rhs.m_vec.begin();
    ...
    */
#endif
} // ibis::bitvector16::xor_c0

// xor operation where rhs is not compressed (not one word is a counter)
void ibis::bitvector16::xor_c1(const ibis::bitvector16& rhs,
    ibis::bitvector16& res) const {
    std::vector<word_t>::const_iterator i0 = m_vec.begin();
    std::vector<word_t>::const_iterator i1 = rhs.m_vec.begin(), i2;
    word_t s0;
    ...
    lg() << "result XOR " << res;
#endif
} // ibis::bitvector16::xor_c1

// bitwise xor (^) operation -- both operands may contain compressed words
void ibis::bitvector16::xor_c2(const ibis::bitvector16& rhs,
    ibis::bitvector16& res) const {
    int bi_idx_pos = 0;
    run16 x, y;
    res.clear();
...
#endif
} // bitvector16& ibis::bitvector16::xor_c2
...
void ibis::bitvector16::xor_d1(const ibis::bitvector16& rhs) {
    // m_vec.nosharing(); // make sure *this is not shared!
#if DEBUG + 0 > 1 || _DEBUG + 0 > 1
    {
        ibis::util::logger lg(4);
...
#endif
} // ibis::bitvector16::xor_d1

// xor operation on two compressed bitvectors, generates uncompressed result
void ibis::bitvector16::xor_d2(const ibis::bitvector16& rhs,
    ibis::bitvector16& res) const {
    // establish a uncompressed bitvector with the right size
    res.nbits = ((nbits == 0 && m_vec.size() > 0) ?
        (rhs.nbits == 0 && rhs.m_vec.size() > 0) ?
