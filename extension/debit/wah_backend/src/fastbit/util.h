...
public:
    ...
    nameList(const char* str) : cstr(0), buff(0) {
        select(str);
    }
    ~nameList() {
        if (cstr) clear();
...
inline void setVerboseLevel(int v) {
    ibis::gVerbose=v;
}
...
public:
    ...
    ~writeLock() {
        ...
        // c++17 complains about the throw-exception-in-destructor statement.
        // if (ierr != 0) {
        //     throw "writeLock failed to release the lock";
        // }
        LOGGER(ibis::gVerbose > 1)
            << "writeLock failed to release the lock";
    }
...
template <class T>
inline refHolder<T> ref(T& r) {
    return refHolder<T>(r);
}
