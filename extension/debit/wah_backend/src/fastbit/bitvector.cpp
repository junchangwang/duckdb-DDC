...
std::vector<uint32_t> ibis::bitvector::decode(std::vector<uint32_t> &append_rowids, Table_config *config) {
    std::vector<uint32_t> rids(m_vec.size());
...
//void ibis::bitvector::improveIndex(word_t ind) {
//    word_t jnd = ind;
//    array_t<word_t>::const_iterator it = m_vec.begin();
//
//    int indPos = -1;
...
//                tmp.push_back(std::make_pair(wordPos, vecPos));
//                index.insert(index.begin() + indPos, tmp.begin(), tmp.end());
//                return;
//            }
//            jnd -= cnt;
//        }
...
void ibis::bitvector::appendActive() {
    if (active.nbits > 0)
        append_active();
}

void ibis::bitvector::buildIndex() {
    int nWords = 0;
    run it;
    it.it = m_vec.begin();
...
void ibis::bitvector::merge(const bitvector &rhs) {
    this->append_active();
    size_t size = m_vec.size();
    m_vec.resize(size + rhs.m_vec.size());
    for (size_t pos = 0; pos < rhs.m_vec.size(); ++pos) {
        m_vec[size + pos] = rhs.m_vec[pos];
    }
}

int ibis::bitvector::getBitWithIndex(const ibis::bitvector::word_t ind) const {
    word_t jnd = ind;
    array_t<word_t>::const_iterator it = m_vec.begin();
...
void ibis::bitvector::compress(float expect_compress_ratio) {
    if (m_vec.size() < 2) // there is nothing to do
        return;

    array_t<word_t> tmp;
    // expect about 38.2% compression ratio
    // but high cardinality bitvectors are expected to have a higher compression ratio
    tmp.reserve(static_cast<uint32_t>(m_vec.size() * expect_compress_ratio));
...
uint64_t ibis::bitvector::do_cnt() const throw() {
    uint64_t ns = 0;
    uint64_t nb = 0;
...
void ibis::bitvector::setBit(const uint64_t ind, int val, Table_config *config) {
    //cout << "32-bit setbit" << endl;
    ...
    if (m_vec.size() * MAXBITS == nbits) { // uncompressed
        ...
        // if (showEB) {
        //     cout << "U EB Decode " << time_diff(before , after) << '\n';
        // }
        ...
        // gettimeofday(&after, NULL);
        // if (showEB) {
        //     cout << "U EB Update " << time_diff(before , after) << '\n';
        // }
    }
...
/// accesses!
int ibis::bitvector::getBit(const uint64_t ind, Table_config *config) const {
    if (ind >= size()) {
        return 0;
    }
    else if (ind >= nbits) {
...
void ibis::bitvector::read_bmz(const char *fn, std::pair<uint32_t, uint32_t> place) {
    if (fn == nullptr || *fn == 0) return;

    uint32_t offset = place.first;
    uint32_t length = place.second;
    if (length == 0) return;

    FILE* bmz = fopen(fn, "rb");
    if (!bmz) {
        LOGGER(ibis::gVerbose > 0)
            << "Warning -- bitvector::read_bmz failed to open \"" << fn << "\"";
        return;
    }

    if (fseek(bmz, offset * sizeof(word_t), SEEK_SET) != 0) {
        fclose(bmz);
        LOGGER(ibis::gVerbose > 0)
            << "Warning -- bitvector::read_bmz failed to seek to offset " << offset;
        return;
    }

    std::vector<word_t> buf(length);
    size_t nread = fread(buf.data(), sizeof(word_t), length, bmz);
    fclose(bmz);
    if (nread != length) {
        LOGGER(ibis::gVerbose > 0)
            << "Warning -- bitvector::read_bmz only read " << nread << " of " << length << " words";
        return;
    }

    m_vec.clear();
    for (uint32_t i = 0; i < length; ++i)
        m_vec.push_back(buf[i]);
